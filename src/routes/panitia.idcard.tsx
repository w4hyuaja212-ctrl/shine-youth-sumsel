...
import { generateIdCards } from "@/lib/pdf.functions";
import { toast } from "sonner";
import { Download, Loader2, Search, IdCard } from "lucide-react";

export const Route = createFileRoute("/panitia/idcard")({
  head: () => ({ meta: [{ title: "Cetak ID Card — Panitia" }] }),
  component: () => <DashboardLayout mode="panitia"><IdCardPage /></DashboardLayout>,
});
...
function IdCardPage() {
  ...
  const print = async (id: string) => {
    setBusy(id);
    try {
      const r = await idFn({ data: { registrationId: id } });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold">Cetak ID Card</h1>
          <p className="text-sm text-muted-foreground">Generate ID Card PDF dengan QR code (A4 portrait, 4 kartu/halaman). Hanya tim <strong>terverifikasi</strong>.</p>
        </div>
      </div>
